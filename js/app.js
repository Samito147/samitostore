...
(() => {
  ...
  /* =========================================================
     ✅ CORREÇÃO PRINCIPAL (DEV x PROD)
     - Em PROD (Vercel): usa /api/ml (mesma origem)
  ...
  const ML_PROXY_ENDPOINT =
    (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1")
      ? `${VERCEL_PROXY_BASE}/api/ml`
      : "/api/ml";

  // ✅ Fallback (mantido como “plano C” — pode falhar por CORS/403)
  ...
  // ✅ Mantido como plano C: pode falhar por CORS/403 (não dependa dele no DEV)
    ...
    if (mlItemId) {
      try {
        ...
        try {
          ...
          // ✅ Plano C: tentar direto (pode falhar por CORS/403)
