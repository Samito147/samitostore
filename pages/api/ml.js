/* =========================================================
   ...
   - ✅ NOVO: Banner "OFERTA RELÂMPAGO" estilo ML
     ✅ Preço atual representa 47% do preço anterior
     ✅ Preço anterior = preçoAtual / 0.47
     ✅ Desconto = 53% OFF
     ✅ "Encerra em" com contador (parametrizável)
   ========================================================= */
...
(() => {
  ...
  // ✅ Endpoint do seu PROXY (Next.js/Serverless). Ex: /api/ml?item=MLB5022231220
  ...
  function getPromoSecondsFromQuery(params) {
    ...
    if (promoEndRaw && String(promoEndRaw).trim()) {
      ...
      if (Number.isFinite(asNum) && asNum > 0) {
        const endMs = asNum > 1e12 ? asNum : asNum * 1000;
  ...
  /**
   * Extrai e normaliza o Item ID do ML a partir de:
   * - "MLB-5022231220" (URL do produto)
   * - "MLB5022231220" (já normal)
   */
  ...
  /**
   * ✅ Busca via PROXY:
   * GET /api/ml?item=MLB5022231220
   * Espera retorno no formato:
   * {
   *   ok: true,
   *   title, price, currency, condition, sold_quantity,
   *   free_shipping, pictures: [], sizes: [], description, permalink
   * }
   */
  ...
  function setChipsFromProxy(data) {
    const chipsWrap = el("chips");
    if (!chipsWrap) return;

    chipsWrap.innerHTML = "";

    // ✅ Chip 1: frete grátis (quando disponível)
  ...
  function setMainImageAndThumbsFromProxy(data) {
  ...
  function setTextsFromProxy(data) {
    ...
    if (subtitleEl) {
      subtitleEl.textContent = "Você concluirá a compra no Mercado Livre com segurança.";
    }

    const priceEl = el("priceValue");
    if (priceEl) priceEl.textContent = Number.isFinite(data?.price) ? formatBRL(data.price) : "—";

    // ✅ NOVO: aplica promo baseada no preço real
    ...
    // Proxy pode devolver category_id se você quiser, mas aqui mantemos como estava (sem inventar).
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // ✅ NOVO: inicia contador da oferta
    ...
    /* ---------------------------------
       ✅ Tentativa de “puxar do ML”
       Você pode usar:
       - ?url=SEU_LINK_DO_PRODUTO (recomendado)
       ou
       - ?item=MLB5022231220
       --------------------------------- */
    ...
    if (mlItemId) {
      try {
        // ✅ Busca via PROXY (em vez de chamar o ML direto)
        const data = await fetchFromProxy(mlItemId);

        // Preenche textos
        setTextsFromProxy(data);

        // Imagens + thumbs
        setMainImageAndThumbsFromProxy(data);

        // Chips (frete grátis, condição, vendidos, tamanhos)
        setChipsFromProxy(data);

        // ✅ ViewContent com dados reais
        ...
        // Se o PROXY falhar, a página continua funcionando (mas sem auto-populate)
        showError(
          "Não consegui puxar os dados automaticamente agora (proxy indisponível). " +
          "A oferta ainda pode ser aberta normalmente no botão."
        );
        hideSkeleton();

        // Sem preço => esconde promo
        const promoBox = el("promoBox");
        if (promoBox) promoBox.style.display = "none";
      }
    } else {
      // Sem item ID: mantém o fluxo por querystring manual
      hideSkeleton();
      pixelTrack("ViewContent", {
